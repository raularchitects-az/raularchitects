...
export default async function EditProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [row, media, revisions] = await Promise.all([getEntity("projects", id), listMedia(), listRevisions("projects", id)]);
  if (!row) notFound();
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-3xl font-semibold">Layihəni redaktə et</h1>
      <ContentForm table="projects" row={row} media={media} afterSaveHref="/admin/projects" />
      {revisions.length > 0 ? (
