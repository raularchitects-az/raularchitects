...
export default async function EditInsightPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [row, mediaResult, revisions] = await Promise.all([
    getEntity("insights", id),
    loadMedia(),
    listRevisions("insights", id),
  ]);
  if (!row) notFound();
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-3xl font-semibold">Insight redaktə et</h1>
      <ContentForm
        table="insights"
        row={row}
        media={mediaResult.items}
        mediaError={mediaResult.error}
        afterSaveHref="/admin/insights"
      />
      {revisions.length > 0 ? (
